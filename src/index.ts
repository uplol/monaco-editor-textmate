import * as monacoNsps from '@uplol/monaco-editor'
...
class TokenizerState implements monacoNsps.languages.IState {
  ...
  public equals(other: monacoNsps.languages.IState): boolean {
    if (!other || !(other instanceof TokenizerState) || other !== this ||
 ...
 *
 * @param monaco monaco namespace this operation should apply to (usually the
 * `monaco` global unless you have some other setup)
 * @param registry TmGrammar `Registry` this wiring should rely on to provide
 * the grammars
 * @param languages `Map` of language ids (string) to TM names (string)
 */
export function wireTmGrammars(
    monaco: typeof monacoNsps, registry: Registry,
    languages: Map<string, string>, editor?: monacoNsps.editor.ICodeEditor) {
    return Promise.all(
    ...
    return {
      endState: new TokenizerState(res.ruleStack),
          tokens: res.tokens.map(
              token => ({
                ...token,
                // TODO: At the moment, monaco-editor doesn't seem to accept
                // array of scopes
                scopes: editor ? TMToMonacoToken(editor, token.scopes) :
